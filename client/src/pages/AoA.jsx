...
const AoA = () => {
  ...
  // Sử dụng một mảng để lưu trạng thái yêu thích của từng sản phẩm
  const [favoriteProducts, setFavoriteProducts] = useState([]); 
  
  const navigate = useNavigate();

  // Tạo hàm lọc sản phẩm Adidas
  const aoAdidasProducts = products.filter(
    (product) => product.category === "áo" && product.brand === "Adidas"
  );
  // Hiển thị sản phẩm áo Adidas trong trang "Áo" của Adidas
  ...
  const filterProducts = () => {
    if (!searchText) {
      return aoAdidasProducts; // Sử dụng danh sách sản phẩm áo của Adidas
    }
    const searchTerm = searchText.toLowerCase();
    return aoAdidasProducts.filter((product) =>
    ...
    <Helmet title="ALLADIDAS">
      <section>
        <div className="banner__adidas">
          ...
          <div className="overlay-text">ÁO ADIDAS</div>
        </div>
        <Container>
